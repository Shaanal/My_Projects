...
class BufferManager{
    ...
    void deleteFile(string tableName, string t);
    void writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount);
    void writePageRow(string tableName, int pageIndex, vector<int> row, int rowCount);
    void clearBuffer();
    vector<vector<int>> readPage(string matrixName, int pageIndex);
};