...
Page BufferManager::getFromPool(string pageName)
{
    ...
    for (auto &page : this->pages)
        if (pageName == page.pageName)
            return page;
    throw runtime_error("Page not found in pool");
}
/**
...
void BufferManager::writePageRow(string tableName, int pageIndex, vector<int> row, int rowCount)
{
    logger.log("BufferManager::writePage");
    Page page = getPage(tableName, pageIndex);
    // Page page(tableName, pageIndex, row, rowCount);
    page;
    page.writerow(row, rowCount);
    // string pageName = "../data/temp/" + tableName + "_Page" + to_string(pageIndex);
    // if (inPool(page.pageName))
    // {
    //     for (auto it = pages.begin(); it != pages.end(); ++it)
    //     {
    //         if (it->pageName == page.pageName)
    //         {
    //             pages.erase(it);
    //             break;
    //         }
...
void BufferManager::deleteFile(string tableName, string t)
{
    logger.log("BufferManager::deleteTempCSVFile");
...
vector<vector<int>> BufferManager::readPage(string matrixName, int pageIndex){
